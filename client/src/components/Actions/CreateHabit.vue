<template>
  <div id="create-habit" class="action-boxes">
    <div class="header">
      <h3>
        <span>{{ title()[0] }}</span
        >{{ title()[1] }} <span>H</span>abit
      </h3>
    </div>
    <form @submit.prevent="addingHabit">
      <input
        type="text"
        v-model="newHabit.content"
        placeholder="Habit"
        name="habit-content"
        required
      />
      <input
        type="text"
        v-model="newHabit.abbr"
        placeholder="Abbreviation (max 2 characters)"
        name="habit-abbreviation"
        maxLength="2"
        required
      />
...
export default {
  ...
  methods: {
    ...
    title() {
      return [
        this.getHabitMode.substring(0, 1),
        this.getHabitMode.substring(1),
      ];
    },
  },
};
