    ...
    <div class="habit-container">
      <div class="wrapper">
        <div class="habit-format dates-container">
          ...
          <div
            v-for="date in displayMonth"
            :key="date"
            class="indicator-box center"
        ...
        <div class="tracker">
          <div v-for="habit in allHabits" :key="habit._id" class="habit-format">
            <div class="habit-content center" :title="habit.content">
              <p
                @click="execute(habit)"
                :class="{ animate: getHabitMode !== 'create' }"
              >
                {{ habit.abbr }}
              </p>
            </div>
            ...
            >
              <v-icon
                ...
                :class="{
                  completed: habit.datesCompleted.includes(date),
                }"
  ...
  methods: {
    ...
    toggleHabit(date, habit) {
      ...
      if (habit.datesCompleted.includes(date)) {
        updatedDates = habit.datesCompleted.filter(
          (existingDates) => existingDates !== date
        );
      } else {
